...
int main(int argc, char * argv[]) {
  ...
  float mean01, mean11;
  int  length, width, ok = 0, stop = 0, iteration = 1, nb_voisin0, nb_voisin1, classe;
  float U0, U1, EG;
  float ** y; /* image a segmenter */
  float ** x; /* champs d'etiquettes a estimer */
  float * VectHist;
  double t0 = 50 /*temperature initiale*/ , tmin = 0.1 /*temperature finale*/ , tp = t0, prob0, prob1, p_total;
  ...
  while (stop == 0) { //iterer jusqu'a => clusters ne bougent plus
    ...
    printf("%d%s%f %f\n", iteration, " update means: ", mean01, mean11);
  ...
  while (tp > tmin) { //iterer tant que la temperature n'a pas atteint la
    EG = 0;
    for (int i = 0; i < length; i++)
      for (int j = 0; j < width; j++) {
    ...
    if (iteration % 10 == 0)
      printf("%s%d%s%f%s%f\n", "iteration= ", iteration, " energie globale= ", EG, " temperature ", tp);

    tp = 0.99 * tp; //decroissance
    iteration++;
  }
